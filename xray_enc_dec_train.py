...
from model import ImageEncoderReportDecoder, ImageEncoderReportDecoderConfig # TODO: mymodel?
...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default = '/home/desin/CS224N/data/chestXray/', help = 'path to images')
    parser.add_argument('--vocab_path', type=str, default = './db_vocab.pkl', help = 'path to vocabulary')
    parser.add_argument('--cv_path', type=str, default = './db_datasets.pkl', help = 'path to cv with dataset')
...
def main():
    ...
    # load vocabulary and dataframes
    #################################
    data_path = args.data_path # "/home/desin/CS224N/data/chestXray/"

    with open(args.vocab_path, "rb") as cache: # "./db_vocab.pkl"
        db_vocab = pickle.load(cache)
        word_2_id = db_vocab["word_2_id"]
        id_2_word = db_vocab["id_2_word"]
    ...
    with open(args.cv_path, "rb") as cache: # "./db_datasets.pkl"
        db_database = pickle.load(cache)
        train_df = db_database["train_df"]
        val_df = db_database["val_df"]
    print('*' * 50)
    ...
    # generate train/validation sets
    #################################
    train_dataset = chestXRayDataset(train_df, data_path, block_size, img_enc_width, img_enc_height, word_2_id,
                                     id_2_word,
                                     tokenizer)
    val_dataset = chestXRayDataset(val_df, data_path, block_size, img_enc_width, img_enc_height, word_2_id, id_2_word,
                                   tokenizer)
