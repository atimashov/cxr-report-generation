...
class ImageEncoderReportDecoder(nn.Module):
    ...
    def decode(self, reps, targets, len_mask, labels):
        ...
        if len_mask != None:
            len_mask = len_mask > 0
            dec_out = self.transformer_decoder(tgt_emb[:, :-1, :], reps, tgt_mask=self.mask[:-1, :-1],
                                               tgt_key_padding_mask=len_mask[:, :-1])
        ...
        for i in range(labels.shape[0]):
            for j in range(i + 1, labels.shape[0]):
                if label_dot[i, j] > float('-inf'):
                    ...
                    diff = diff - label_dot[i, j] * torch.log(reps_dot) - (1 - label_dot[i, j]) * torch.log(
                        1 - reps_dot)
                    num_diff = num_diff + 1
